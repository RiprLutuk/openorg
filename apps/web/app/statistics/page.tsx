...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Dashboard publik indikator perkembangan sektor, sebaran teknisi BNSP, volume servis terverifikasi, dan metrik kepercayaan publik.",
  };
}
      ...
      <section className="statistics-hero">
        <div className="wrap">
          ...
          <p className="hero-lead">
            Metrik data nasional pertumbuhan jumlah teknisi bersertifikat,
            sebaran kepengurusan DPD 38 Provinsi, serta indeks kepuasan publik
            terhadap layanan terverifikasi.
          </p>
        </div>
      </section>
          ...
          <div className="stats-metric-grid">
            {statsList.length > 0 ? (
              statsList.map((stat) => (
                <div key={stat.id} className="stat-metric-card">
                  <div className="stat-card-top">
                    ...
                    {stat.period && (
                      <span className="stat-period-tag">{stat.period}</span>
                    )}
                  </div>
                  ...
                  <div className="stat-value-wrap">
                    ...
                    {stat.metricUnit && (
                      <span className="stat-unit">{stat.metricUnit}</span>
                    )}
                  </div>
                  {stat.trendPercentage && (
                    <div className={`stat-trend trend-${stat.trendDirection}`}>
                      <TrendingUp size={14} />
                      <span>
                        {stat.trendPercentage} dari kuartal sebelumnya
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="empty-state">
                ...
                <p>
                  Metrik data industri akan dipublikasikan secara berkala oleh
                  sekretariat pusat.
                </p>
              </div>
            )}
